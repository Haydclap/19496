...
def CalcMetrics(i):
    ...
    df = pd.read_csv(f'FILE_NAME/TestDataZero/RFE{i}.csv')
    ...
    indices_to_drop = y_train[y_train == 'na'].index
    X_train_drop = df.drop(index=indices_to_drop)
    y_train_drop = y_train.drop(index=indices_to_drop)
    
    # indices_to_drop = y_train_drop[y_train_drop == 'SSH-Patator'].index
    # X_train_drop = X_train_drop.drop(index=indices_to_drop)
    # y_train_drop = y_train_drop.drop(index=indices_to_drop)

    # indices_to_drop = y_train_drop[y_train_drop == 'FTP-Patator'].index
    # X_train_drop = X_train_drop.drop(index=indices_to_drop)
    # y_train_drop = y_train_drop.drop(index=indices_to_drop)
...
df.to_csv('FILENAME/TestDataZero/feature_all_together.csv', index=False)
